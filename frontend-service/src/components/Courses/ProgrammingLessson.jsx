...
class ProgrammingLessson extends Component {
    ...
    render() {
        ...
        (this.props.programmingLessons.map(({topic,lessonId,youTubeUrl,textDescription}) => {
            if (checkRouteId && lessonId===lessonIdCheck){
                checkRoute=true;  }}))
                    if(checkRoute) return (this.props.programmingLessons.map(({topic,lessonId,youTubeUrl,textDescription}) => {
...
export default withRouter( connect(mapStateToProps, mapDispatchToProps)(ProgrammingLessson));